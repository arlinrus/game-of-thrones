...
plot(
  g1,
  layout = layout1,
  vertex.color = as.factor(comm1), # цвет = сообщество Louvain
  vertex.size = 3 + log1p(deg1) * 2, # размер по степени (лог-масштаб)
  vertex.label = NA,
  edge.width = 0.5 + (E(g1)$weight / max(E(g1)$weight)) * 2,
  main = "Сообщества персонажей (книга 1, Louvain)"
)
```
...
set.seed(123)
layout5 <- layout_with_fr(g5)
deg5 <- degree(g5)

plot(
  g5,
  layout = layout5,
  vertex.color = as.factor(comm5),            # цвет = сообщество Louvain
  vertex.size = 3 + log1p(deg5) * 2,         # размер по степени
  vertex.label = NA,
  edge.width = 0.5 + (E(g5)$weight / max(E(g5)$weight)) * 2,
  main = "Сообщества персонажей (книга 5, Louvain)"
)
```
...
❗️Сгенерил чатик (можно перефразировать и кайф):

Наш проект был посвящён анализу сети персонажей цикла «Песнь Льда и Пламени» на основе данных о совместных появлениях в главах пяти книг. Мы рассматривали персонажей как узлы сети, а их совместные появления — как рёбра, дополнительно учитывая вес связи. Такой подход позволяет взглянуть на знакомый сюжет не как на набор отдельных линий, а как на единую сложную систему взаимодействий.

Во-первых, сетевой анализ подтвердил интуитивное ощущение того, что мир «Игры престолов» сильно фрагментирован. Даже в первой книге персонажи распределены по нескольким относительно автономным кластерам, которые хорошо совпадают с ключевыми сюжетными арками: Север и дом Старков, королевский двор и Ланнистеры, восточная линия Дейенерис. Алгоритмы кластеризации (в частности, Louvain) автоматически выделяют эти группы, что говорит о том, что структура повествования достаточно чётко зашита в паттерны взаимодействий персонажей.

Во-вторых, мы увидели, как меняется состав центральных узлов при переходе от книги к книге. На ранних этапах историю структурируют Эддард Старк и Роберт Баратеон, вокруг которых вращается политический порядок старого режима. Постепенно их место занимают Тирион Ланнистер, Джон Сноу, Дейенерис Таргариен и Станнис Баратеон. Это отражает не только сюжетные повороты (смерти и перевороты), но и глубокую перестройку сети власти: центр тяжести смещается от прежних элит к новым игрокам.

В-третьих, сетевые метрики показывают, что важность персонажа не всегда напрямую связана с количеством его появлений. Некоторые герои с умеренным числом контактов оказываются критически важными посредниками, через которых проходят кратчайшие пути между кластерами. Такие узлы, обладающие высокой betweenness centrality, можно рассматривать как точки потенциального «разрыва» сети: их устранение теоретически способно сильно фрагментировать повествовательное пространство.

С практической точки зрения подобный анализ можно применять не только к художественным произведениям, но и к другим сложным системам — от корпоративных структур до социальных движений. Работа с относительно безопасным и понятным кейсом вроде «Игры престолов» позволяет освоить инструменты сетевого анализа, интерпретацию метрик и базовые подходы к работе с данными. В дальнейшем ту же логику можно перенести на реальные социальные сети, профессиональные коммуникации или политические взаимодействия.

Наш проект можно развить несколькими способами. Например, добавить эмоциональную окраску связей (дружба, конфликт, нейтральное взаимодействие), построить динамическую сеть по главам или сравнить книжную структуру с сериалом HBO. Это позволило бы ещё точнее понять, как выбор авторов и сценаристов влияет на архитектуру мира и восприятие зрителями ключевых персонажей.
...
- *Астапенкова Арина* отвечала за очистку данных, построение графов для каждой книги, расчёт основных сетевых показателей и подготовку визуализаций.
- *Горчинская Арина* занималась поиском и предварительным изучением датасета, описанием предметной области, а также написанием текстовых интерпретаций метрик центральности для ключевых персонажей.
- *Турутин Никита* реализовал кластерный анализ, сравнение модулярности, а также помогал со структурированием отчёта и формулировкой общих выводов.
