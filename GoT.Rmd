...
<div style="display:flex; justify-content:center; gap:15px; margin:20px 0; flex-wrap:nowrap;">
...
<div style="display:flex; justify-content:center; gap:15px; margin:20px 0; flex-wrap:nowrap;">
  ...
  <figure style="text-align:center; margin:0;">
    <img src="images/renly.jpg" style="width:130px; border-radius:12px;">
    <figcaption style="margin-top:6px; font-size:14px; color:#555;">
      Ренли Баратеон
    </figcaption>
  </figure>
...
4.  **Джейме Ланнистер** — претендент на трон, объединяющий северные и южные ветки.
5.  **Станнис Баратеон** — дипломатический узел, соединяющий несколько домов.
...
1.  **Джейми Ланнистер** — активно перемещается и взаимодействует с разными домами.
...
1.  **Серсея Ланнистер** — центр всех дворцовых интриг, через неё проходят связи между королевской семьёй, церковью и армией
...
3.  **Станнис Баратеон** — единственный претендент на трон, чьи действия связывают Север, Драконий Камень и южные дома — ключевой узел в борьбе за власть.
4.  **Сэмвел Тарли** — мост между учёным миром (Мейстеры), Стражей Ночи и внешними угрозами его знания и путь делают его неожиданным посредником.
5.  **Бриенна Тарт** — связывает Север, королевский двор и рыцарские кодексы её лояльность и перемещения создают редкие, но важные пути между группами.
...
<div style="display:flex; justify-content:center; gap:15px; margin-top:20px; flex-wrap:nowrap;">
  ...
  <figure style="text-align:center;">
    <img src="images/samwell.jpg" style="width:130px; border-radius:12px;">
    <figcaption style="margin-top:6px; font-size:14px; color:#555;">
      Сэмвел Тарли
    </figcaption>
  </figure>
...
```{r}
bet5 <- head(sort(betweenness(g5, directed = F, weights = 1/E(g5)$weight), decreasing = TRUE), 5)
bet5
```
...
2.  **Тирион Ланнистер** — продолжает быть посредником, но уже на другом уровне между Дейнерис, драконами и старым миром. Его ум — главный канал обмена.
...
4.  **Роберт Баратеон** — хотя он мёртв, его имя и наследие продолжают влиять его фигура остаётся символическим узлом, вокруг которого строятся новые союзы.
5.  **Дейенерис Таргариен** — центр новой империи, её решения связывают драконов, жителей Мира, наёмников и вассалов — она становится глобальным посредником.
...
![](images/big_tyrion.jpg){style="border-radius:12px;" width="300"}
...
ggplot(house_summary, aes(x = reorder(house, total_strength), y = total_strength)) +
  geom_col(fill = "#f7d488", color = "#2b2b2b", size = 1.5) +
  coord_flip() +
  labs(
...
Обратим внимание, что other - это персонажи, которые не относятся ни к одному из домов, поэтому его мы не считаем. Самый активный дом в 1 книге, конечно, **Старки**, так как там появляется Робб Старк и идет упор на раскрытие сюжета дома старков.

<figure style="text-align:center;">

![](images/stark.jpg){style="border-radius:12px;"}
...
ggplot(house_summary, aes(x = reorder(house, total_strength), y = total_strength)) +
  geom_col(fill = "#93b2bd", color = "#2b2b2b", size = 1.5) +
  coord_flip() +
  labs(
...
На первый план в 5 книге вышли **Таргариены**. Интересно, что из семейства таргариенов в основном участвует Дейенерис. Отец - Эйрис II Таргариен несколько раз упоминается в качестве родного отца Джона Сноу.

<figure style="text-align:center;">

![](images/targarien.jpg){style="border-radius:12px;"}
