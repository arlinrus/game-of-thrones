...
<img src="images/poster.jpeg" style="width:300px; border-radius:12px;"/>
...
"Средняя длина пути" = mean_distance(.x, directed = FALSE, unconnected = TRUE, weights = E(.x)$weight)
))

kable(summary_tbl, digits = 3, caption = "Описательные характеристики сети по книгам")
```

-   **Узлы**: количество растет от книги к книге (с 187 в первой книге до 317 в пятой) =\> отражает расширение мира: появляются новые линии, новые дома, новые второстепенные герои.
...
deg1
```
Мы используем невзвешенный граф, потому что хотим понять сколько связей имеют персонажи.
...
::: {style="display:flex; justify-content:center; gap:15px; margin-top:20px; flex-wrap:nowrap;"}
<figure style="text-align:center;">

<img src="images/eddard.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/robert.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/tyrion.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/catelyn.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/jon.jpeg" style="width:130px; border-radius:12px;"/>
...
! У нас вес - это количество появлений двух персонажей вместе, значит, чем больше вес, тем вероятнее, что персонажи вместе, поэтому мы делим 1 на веса, чтобы превратить "силу связи" в "длину пути", потому что алгоритмы поиска кратчайших путей ожидают, что веса = стоимость прохождения по ребру, а не мера близости.

```{r}
bet1 <- head(sort(betweenness(g1, directed = FALSE, weights = 1 / E(g1)$weight), decreasing = TRUE), 5)
bet1
```

Наиболее часто находятся на кратчайших путях между другими героями, что делает их ключевыми посредниками в сети взаимодействий:

1.  **Эддард старк** — выступает ключевым связующим звеном между Севером, Королевской Гаванью и военными/политическими элитами. Его высокая битвинность отражает не только социальный статус, но и позицию посредника в условиях растущего политического напряжения.

2.  **Кейтилин Старк** — имеет высокую битвинность благодаря своей способности связывать несколько изолированных подсетей.

3.  **Джон Сноу** —  связан со Старками (через Эддарда и Робба) и одновременно с Севером, Ночным Дозором и даже дикими людьми (в зародыше).

4.  **Роберт Баратеон** — связан со всеми ведущими домами: со Старками (через дружбу с Эддардом), с Ланнистерами (через брак), с Дорном и Речными землями (как верховный правитель).

5.  **Тирион Ланистер** — самый общительный среди семейства Ланнистеров.
...
<img src="images/robb.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/jaime.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/loras.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/joffrey.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/cersei.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/arya.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/stannis.jpeg" style="width:130px; border-radius:12px;"/>
...
```{r}
bet2 <- head(sort(betweenness(g2, directed = FALSE, weights = 1 / E(g2)$weight), decreasing = TRUE), 5)
bet2
```

Наиболее часто находятся на кратчайших путях между другими героями, что делает их ключевыми посредниками в сети взаимодействий:

1.  **Робб Старк** — главный мост между Севером и союзниками. Управляет потоками власти и войск
2.  **Тирион Ланнистер** — посредник между двором, армией и интригами. Соединяет противоборствующие стороны.
3.  **Джоффри Баратеон** — формальный центр сети все пути ведут к нему (или от него), даже если он не управляет ими.
4.  **Арья Старк** — мост между элитой и низами, Севером и чужими землями. Её путь связывает разные миры.
5.  **Ренли Баратеон** — объединяет юг и двор. Его союзы создают новые информационные каналы между домами.
...
<img src="images/sansa.jpeg" style="width:130px; border-radius:12px;"/>
...
```{r}
bet3 <- head(sort(betweenness(g3, directed = F, weights = 1 / E(g3)$weight), decreasing = TRUE), 5)
bet3
```

Наиболее часто находятся на кратчайших путях между другими героями, что делает их ключевыми посредниками в сети взаимодействий:

1.  **Робб Старк** — центр столичной политики, соединяющий множество фракций.
2.  **Тирион Ланистер** — остаётся структурно важным через связи и последствия его правления.
3.  **Джон Сноу** — соединяет линии Стены, одичалых и южные события.
4.  **Джейми Ланнистер** — претендент на трон, объединяющий северные и южные ветки.
5.  **Станис Баратеон** — дипломатический узел, соединяющий несколько домов.
...
<img src="images/brienne.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/margaery.jpeg" style="width:130px; border-radius:12px;"/>
...
```{r}
bet4 <- head(sort(betweenness(g4, directed = F, weights = 1/ E(g4)$weight), decreasing = TRUE), 5)
bet4
```

Наиболее часто находятся на кратчайших путях между другими героями, что делает их ключевыми посредниками в сети взаимодействий:

1.  **Церсея Ланнистер** — центр всех дворцовых интриг, через неё проходят связи между королевской семьёй, церковью и армией
2.  **Джейме Ланнистер** — перемещается между военными лагерями, семьёй и политическими противниками — становится мостом между силовыми блоками.
3.  **Станнис Барате** — единственный претендент на трон, чьи действия связывают Север, Драконий Камень и южные дома — ключевой узел в борьбе за власть.
4.  **Самвел Тарли** — мост между учёным миром (Мейстеры), Стражей Ночи и внешними угрозами его знания и путь делают его неожиданным посредником.
5.  **Бринея Тарт** — связывает Север, королевский двор и рыцарские кодексы её лояльность и перемещения создают редкие, но важные пути между группами.

::: {style="display:flонex; justify-content:center; gap:15px; margin-top:20px; flex-wrap:nowrap;"}
<figure style="text-align:center;">
...
<img src="images/balon.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/baelor.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/tywin.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/daenerys.jpeg" style="width:130px; border-radius:12px;"/>
...
<img src="images/theon.jpeg" style="width:130px; border-radius:12px;"/>
...
```{r}
bet5 <- head(sort(betweenness(g5, directed = F, weights = 1/E(g5)$wieght), decreasing = TRUE), 5)
bet5
```

Наиболее часто находятся на кратчайших путях между другими героями, что делает их ключевыми посредниками в сети взаимодействий:

1.  **Станнис Баратеон** — остаётся ключевым узлом: его кампания объединяет Север, рыцарей и верующих — все пути ведут к нему или через него.
2.  **Дейенерис Таргариен** — центр новой империи, её решения связывают драконов, жителей Мира, наёмников и вассалов — она становится глобальным посредником.
3.  **Джон Сноу** — на границе мира живых и мёртвых, его положение на Стене и связь с Севером делают его мостом между двумя мирами.
4.  **Тирион Ланнистер** — продолжает быть посредником, но уже на другом уровне между Дейнерис, драконами и старым миром. Его ум — главный канал обмена.
5.  **Роберт Баратеон** — хотя он мёртв, его имя и наследие продолжают влиять его фигура остаётся символическим узлом, вокруг которого строятся новые союзы.
...
<img src="images/asha.jpeg" style="width:130px; border-radius:12px;"/>
...
### Битвинность Тириона Ланнистера

```{r}
t1 <- betweenness(g1, directed = FALSE, weights = 1 / E(g1)$weight)["Tyrion-Lannister"]
t2 <- betweenness(g2, directed = FALSE, weights = 1 / E(g2)$weight)["Tyrion-Lannister"]
t3 <- betweenness(g3, directed = FALSE, weights = 1 / E(g3)$weight)["Tyrion-Lannister"]
t4 <- betweenness(g4, directed = FALSE, weights = 1 / E(g4)$weight)["Tyrion-Lannister"]
t5 <- betweenness(g5, directed = FALSE, weights = 1 / E(g5)$weight)["Tyrion-Lannister"]

df <- tibble(
  Книга = 1:5,
  Битвинность = c(t1, t2, t3, t4, t5)
)

ggplot(df, aes(x = Книга, y = Битвинность)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  labs(
    title = "Тириона Ланнистера по книгам",
    x = "Номер книги",
    y = "Битвинность"
  ) +
  theme_minimal()
...
**Книги 1–3**: Тирион активно участвует в политических интригах Королевской Гавани, выступая мостом между двором, армией и Севером.

**Книга 4**: Резкое падение связано с его бегством из Вестероса. Он временно исчезает из основной сети, теряя связи с ключевыми фигурами.

**Книга 5**: Резкий скачок отражает его новую роль в мире, он становится посредником между Дейнерис Таргариен, драконами и старым миром, что делает его ещё более важным узлом, чем раньше.


### Какой дом чаще всего взаимодействует с другими?

##### Книга 1
```{r}
V(g1)$base_house <- sapply(strsplit(V(g1)$name, "-"), function(x) {
  if (length(x) >= 2) x[2] else x[1]
})
V(g1)$base_house <- gsub("\\(.*", "", V(g1)$base_house)
V(g1)$base_house <- trimws(V(g1)$base_house)
...
house_map <- c(
  ...
  "Manderly" = "Stark",
  "Reed" = "Stark",
  "Darry" = "Stark",  # хотя формально Речные земли — но лояльны Старкам в книге 1
  ...
  "Tully" = "Tully",
  "Blackwood" = "Tully",
  "Bracken" = "Tully",  # спорно, но в книге 1 они часть коалиции Талли
  "Mallister" = "Tully", # если появится
  ...
  # Штормовые земли → Баратеоны
  "Baratheon" = "Baratheon",
  
  # Простор → Тиреллы
  "Tyrell" = "Tyrell",
  "Redwyne" = "Tyrell",
  "Tarly" = "Tyrell",  # формально, но в книге 1 они лояльны Тиреллам
  
  # Дорн → Мартеллы
  "Martell" = "Martell",  # в книге 1 почти нет упоминаний → может быть 0
  "Dayne" = "Martell",
  
  # Железные острова → Грейджои
  "Greyjoy" = "Greyjoy",
...
# Применяем маппинг
V(g1)$house <- unname(sapply(V(g1)$base_house, function(h) {
  if (h %in% names(house_map)) {
    house_map[h]
  } else {
    "Other"
  }
...
V(g1)$strength <- strength(g1, weights = E(g1)$weight)

house_summary <- tibble(
  house = V(g1)$house,
  strength = V(g1)$strength
) %>%
  group_by(house) %>%
  summarise(total_strength = sum(strength, na.rm = TRUE)) |>
  arrange(desc(total_strength))

target_houses <- c("Stark", "Lannister", "Baratheon", "Tully", "Arryn", 
                   "Tyrell", "Greyjoy", "Targaryen", "Martell")
house_summary <- house_summary |>
  filter(house %in% c(target_houses, "Other")) |>
  mutate(house = factor(house, levels = c(target_houses, "Other")))

# График
ggplot(house_summary, aes(x = reorder(house, total_strength), y = total_strength)) +
  geom_col(fill = "purple", color = "darkblue", size = 1.5) +
  coord_flip() +
  labs(
    title = "Активность главных домов в Книге 1",
    subtitle = "Частота взаимодействий",
    x = "Дом",
    y = "Общая активность"
  ) +
  theme_minimal()
...
Обратим внимание, что other - это персонажи, которые не относятся ни к одному из домов, поэтому его мы не считаем.
Самый активнй в 1 книге, конечно старки, так как там появляется Роб Старк и идет упор на раскрытие сюжета дома старков.

#### Книга 5

```{r}
V(g5)$base_house <- sapply(strsplit(V(g5)$name, "-"), function(x) {
  if (length(x) >= 2) x[2] else x[1]
})
V(g5)$base_house <- gsub("\\(.*", "", V(g5)$base_house)
V(g5)$base_house <- trimws(V(g5)$base_house)
...
# Применяем маппинг
V(g5)$house <- unname(sapply(V(g5)$base_house, function(h) {
  if (h %in% names(house_map)) {
    house_map[h]
  } else {
    "Other"
...
V(g5)$strength <- strength(g5, weights = E(g5)$weight)

house_summary <- tibble(
  house = V(g5)$house,
  strength = V(g5)$strength
) |>
  group_by(house) |>
  summarise(total_strength = sum(strength, na.rm = TRUE)) |>
  arrange(desc(total_strength))
...
ggplot(house_summary, aes(x = reorder(house, total_strength), y = total_strength)) +
  geom_col(fill = "lightblue", color = "darkblue", size = 1.5) +
  coord_flip() +
  labs(
    title = "Активность главных домов в Книге 5",
    subtitle = "Частота взаимодействий",
    x = "Дом",
    y = "Общая активность"
  ) +
...
Также не учитываем other, теперь на первый план в 5 книге у нас вышли Таргариены. Интересно, что из семейства таргариенов в основном участвует Дайнерис, как один персонад, отец - Эйрис II Таргариен несколько раз упоминается в качестве родного отца Джона Сноу. Тут конечно много спойлеров, так что извиняемся.
...
# Функция для сравнения алгоритмов кластеризации по одному графу
algo <- function(g) {
  set.seed(123)
  louvain_comm <- cluster_louvain(g, weights = E(g)$weight)
  fg_comm      <- cluster_fast_greedy(g, weights = E(g)$weight)
  wt_comm      <- cluster_walktrap(g,  weights = E(g)$weight)
  lp_comm      <- cluster_label_prop(g, weights = E(g)$weight)
  im_comm      <- cluster_infomap(g,   e.weights = E(g)$weight)
  
  tibble(
    Algorithm = c("Louvain", "Fast Greedy", "Walktrap", "Infomap", "Label Prop"),
    Modularity = c(
    ...
    Communities = c(
      length(unique(membership(louvain_comm))),
      length(unique(membership(wt_comm))),
      length(unique(membership(fg_comm))),
      length(unique(membership(im_comm))),
      length(unique(membership(lp_comm)))
    )
  ) |> arrange(desc(Modularity))
}

# Сводная таблица по всем книгам
results <- purrr::map_dfr(graphs, ~ algo(.x), .id = "Book")

knitr::kable(
  results,
  digits = 3,
  caption = "Сравнение алгоритмов кластеризации по 5 книгам"
)
```

Сравнение алгоритмов показывает, что *Louvain* и *Walktrap* стабильно дают наибольшую модулярность по всем пяти книгам. Это означает, что они лучше остальных разделяют сеть на структурно плотные сообщества.
...
В пятой книге выделено **11 сообществ** при модулярности **0.706**. Это ещё более высокое значение, чем в первой книге, что говорит о очень чётко выраженной разбиении сети на отдельные группы:
...
В *первой книге* сообщества выглядят более компактными и крупными: персонажи чётко сгруппированы по нескольким основным сюжетным линиям, и связи между кластерами относительно редки.
...
Если расширять наш проект, то можно добавить эмоциональную окраску связей (дружба, конфликт, нейтральное взаимодействие). Также можно построить сеть по каждой главе, чтобы детальнее рассмотреть, как меняется сюжет. Еще можно сравнить структуру книг и сериала от HBO, чтобы посмотреть какие решения авторов и сценаристов влияют на мир и в чем разница между киновселенной и оригинальной вселенной.
...
<img src="images/Jon Snow.jpg" style="width:300px; border-radius:12px;"/>

<figcaption style="margin-top:6px; font-size:20px; color:#555;">
